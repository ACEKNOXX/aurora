...
class selector {
    ...
    public function select_all($table){
        ...
        $i=0;
         while($fetch = mysqli_fetch_assoc($connect)){
            $tab_array[$i] = $fetch;
            $i++;
        }
        return  json_encode($tab_array);

    }
    public function select_with_limit($table,$where = "1=1",$lim){
        $query="SELECT * FROM ".$table." WHERE ".$where." ORDER BY id DESC LIMIT ".$lim." ";
        $connect = mysqli_query($this->conn, $query);
         while($fetch = mysqli_fetch_assoc($connect)){
            $tab_array[$i] = $fetch;
            $i++;
    ...
    public function select_it($table,$where = "1=1", $data="*"){
        $query = "SELECT ".$data." FROM ".$table. " WHERE ".$where." ORDER BY id DESC";
        $connect = mysqli_query($this->conn, $query);
        return $connect;
    }
